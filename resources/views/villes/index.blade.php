                      ...
                      <tr>
                        ...
                        >
                          <a
                            ...
                            onclick="openDropdown(event,'table-light-{{ $ville->id }}-dropdown')"
                          >
                            <i class="fas fa-ellipsis-v"></i>
                          </a>
                          <div
                            class="hidden bg-white text-base z-50 float-left py-2 list-none text-left rounded shadow-lg min-w-48"
                            id="table-light-{{ $ville->id }}-dropdown"
                          >
                            <a
                              href="{{ route('villes.show',  $ville->id) }}"
...
<x-app-layout>
    <x-slot name="header">
        <h2>Pharmacies de votre ville</h2>
        <a href="{{ route('pharmacies.create') }}" class="btn btn-primary">Ajouter une pharmacie</a>
    </x-slot>
        ...
        <table class="table-auto w-full border">
            ...
            <tbody>
                @foreach($pharmacies as $pharmacie)
                    <tr>
                        <td class="border px-4 py-2">{{ $pharmacie->name_pharmacie }}</td>
                        <td class="border px-4 py-2">{{ $pharmacie->commune->name_commune }}</td>
                        <td class="border px-4 py-2">
                            <a href="{{ route('pharmacies.edit', $pharmacie->id) }}" class="btn btn-sm btn-warning">Editer</a>

                            <form action="{{ route('pharmacies.destroy', $pharmacie->id) }}" method="POST" class="inline-block">
                                @csrf
                                @method('DELETE')
